...
import com.example.flexie.screens.ChatScreens.actualChatScreen
import com.example.flexie.screens.ChatScreens.addPeopleScreen
...
fun BottomNav_navgraph(navController: NavHostController, modifier: Modifier) {
    ...
    var playerViewModel: PlayerViewModel = hiltViewModel()
    var sharedViewModelMovie: SharedViewModelMovie = hiltViewModel()
    ...
    NavHost(navController = navController, startDestination = screen.Home.route) {
        ...
        composable(screen.Search.route) {
            Search_screen(searchViewModel, navController)
        }
        composable(screen.Chat.route) {
            chat_screen(navController, chatScreenViewModel)
        }
        ...
        composable("addPeople") {
            addPeopleScreen(searchPeopleViewModel, navController)
        }
        composable(
            "actualChat/{id}/{name}",
            arguments = listOf(navArgument("id") { type = NavType.StringType } , navArgument("name"){type = NavType.StringType})
        ) {
            val id = it.arguments?.getString("id")?:""
            val name = it.arguments?.getString("name")?:""
            val actualChatViewModel : ActualChatViewModel = hiltViewModel()
            actualChatScreen(id , name , actualChatViewModel , navController)
        }
    }
}
...
sealed class screen(val route: String, val name: String, val icon: ImageVector) {
    ...
    object Chat : screen("chat", "Chats", Icons.Rounded.Chat)
    object Friends : screen("friends", "Friends", Icons.Rounded.Groups)
    object Profile : screen("profile", "Profile", Icons.Rounded.AccountCircle)
}