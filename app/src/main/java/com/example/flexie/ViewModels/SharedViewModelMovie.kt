...
@HiltViewModel
class SharedViewModelMovie @Inject constructor (private val homeMovieCategoryRepository: home_movie_category_repository , private val continueWatchingRepository: ContinueWatchingRepository) : ViewModel() {
    var movieData : movie_detail_item? = null

     var moreLikeThis : MutableStateFlow<List<movie_home_row>> = MutableStateFlow<List<movie_home_row>>(
        emptyList()
    )
    ...
    fun addWatching(id : String){
        viewModelScope.launch {
            continueWatchingRepository.addToContinueWatching(id)
        }
    }
